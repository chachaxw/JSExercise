...
/***无法顺序执行的解决办法是为每一个事件分配一个计数器，实现累加，并清晰的指定给addEvent使用,js一切皆为对象,所以addEvent.ID正确，实际上是一个全局变量。***/
//跨浏览器事件绑定--IE浏览器与W3C的标准的区别：
function addEvent(obj,type,fn){
	if(typeof addEventListener!='undefined')
	{
		obj.addEventListener(type,fn,false);//第三个参数为true时，表示为捕获阶段；为false时，表示为冒泡阶段。一般使用false,原因是IE浏览器，不支持捕获阶段的事件监听。
	}
